export function formatAiContextUsageTooltip(usage) {
  ...
  const outputTokens = Math.max(0, Math.round(Number(usage.effectiveOutputTokens ?? usage.maxOutputTokens ?? usage.outputReserveTokens) || 0)).toLocaleString("zh-CN");
  return `总输入 ${inputTokens} / ${contextWindow} tok · 作品上下文 ${contextTokens} tok · 对话历史 ${conversationTokens} / ${conversationBudget} tok · 本次最大输出 ${outputTokens} tok`;
}
...
export function normalizeAiContextTokenDistribution(usage) {
  ...
  const outputTokens = Math.min(
    tokenCount(distribution.outputTokens ?? usage?.effectiveOutputTokens ?? usage?.maxOutputTokens ?? usage?.outputReserveTokens),
