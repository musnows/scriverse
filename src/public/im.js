...
export function createImWorkspace({ api, esc, renderMarkdown, toast, state, showShelf }) {
  ...
  function renderDetails() {
    ...
    const humanRows = activeHumans().map((item) => `<li><span>${esc(item.displayName)} <small>@${esc(item.username)}</small>${item.role === "owner" ? " · 群主" : ""}</span>${owner && item.userId !== currentUserId() ? `<button type="button" data-im-remove-human="${esc(item.userId)}" aria-label="移除 ${esc(item.displayName)}">移除</button>` : ""}</li>`).join("");
    const characterRows = activeCharacters().map((item) => `<li><span>${esc(item.name)} <small>${esc(item.workTitle)}</small></span>${owner && current.kind === "group" && activeCharacters().length > 1 ? `<button type="button" data-im-remove-character="${esc(item.characterId)}" aria-label="移除 ${esc(item.name)}">移除</button>` : ""}</li>`).join("");
