...
const IM_SCHEMA_SQL = `
  ...
  CREATE INDEX IF NOT EXISTS idx_im_messages_human_avatar
    ON im_messages(
      ...
      COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
        CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
          THEN substr(json_extract(sender_snapshot_json, '$.avatarUrl'), instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') + 3) END),
      sequence
    ) WHERE sender_kind = 'human';
  CREATE INDEX IF NOT EXISTS idx_im_messages_character_avatar
    ON im_messages(
      conversation_id, sender_character_id,
      COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
        CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
          THEN substr(json_extract(sender_snapshot_json, '$.avatarUrl'), instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') + 3) END),
      sequence
    ) WHERE sender_kind = 'character';
  CREATE INDEX IF NOT EXISTS idx_im_messages_character_snapshot_avatar
    ON im_messages(
      conversation_id, json_extract(sender_snapshot_json, '$.id'),
      COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
        CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
          THEN substr(json_extract(sender_snapshot_json, '$.avatarUrl'), instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') + 3) END),
      sequence
  ...
  CREATE TABLE IF NOT EXISTS im_chain_turns (
    ...
    character_membership_id TEXT NOT NULL REFERENCES im_character_memberships(id) ON DELETE CASCADE,
    source_message_id TEXT REFERENCES im_messages(id) ON DELETE SET NULL,
  ...
  private migrate(): void {
    this.raw.exec(`
      ...
      CREATE TABLE IF NOT EXISTS platform_ai_settings (
        ...
        system_prompt_override_enabled INTEGER NOT NULL DEFAULT 0 CHECK(system_prompt_override_enabled IN (0, 1)),
  ...
  private applyDataMigrations(): void {
    ...
    if (!applied.has(122) || !aiWritePlanTablesPresent) {
      this.transaction(() => {
        ...
        // AskUserQuestions 提问记录：一次工具调用对应一个问题批次，持久化保存，支持刷新后继续查看。
        this.run(`
    ...
    const imMessageAvatarHashIndexesPresent = [
      "idx_im_messages_human_avatar",
      "idx_im_messages_character_avatar",
      "idx_im_messages_character_snapshot_avatar"
    ].every((index) => String(this.get(
      "SELECT sql FROM sqlite_master WHERE type = 'index' AND name = ?",
      index
    )?.sql ?? "").includes("avatarSha256"));
    if (!applied.has(134) || !imMessageAvatarHashIndexesPresent) {
      this.transaction(() => {
        this.run("DROP INDEX IF EXISTS idx_im_messages_human_avatar");
        this.run("DROP INDEX IF EXISTS idx_im_messages_character_avatar");
        this.run("DROP INDEX IF EXISTS idx_im_messages_character_snapshot_avatar");
        this.raw.exec(`CREATE INDEX idx_im_messages_human_avatar
          ON im_messages(
            conversation_id, sender_user_id,
            COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
              CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
        ...
        CREATE INDEX idx_im_messages_character_avatar
          ON im_messages(
            conversation_id, sender_character_id,
            COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
              CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
        ...
        CREATE INDEX idx_im_messages_character_snapshot_avatar
          ON im_messages(
            conversation_id, json_extract(sender_snapshot_json, '$.id'),
            COALESCE(NULLIF(json_extract(sender_snapshot_json, '$.avatarSha256'), ''),
              CASE WHEN instr(json_extract(sender_snapshot_json, '$.avatarUrl'), '?v=') > 0
        ...
        this.run("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (134, ?)", new Date().toISOString());
      });
      const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
      if (integrity.some((row) => row.integrity_check !== "ok")) {
        throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
    ...
    const aiQuestionColumns = new Set(
      this.all<{ name: string }>("PRAGMA table_info(ai_user_questions)").map((column) => column.name)
    );
    if (!applied.has(135) || !aiQuestionColumns.has("questions_json") || !aiQuestionColumns.has("answers_json")) {
      this.transaction(() => {
        if (!aiQuestionColumns.has("questions_json")) {
          this.run("ALTER TABLE ai_user_questions ADD COLUMN questions_json TEXT NOT NULL DEFAULT '[]'");
        }
        if (!aiQuestionColumns.has("answers_json")) {
          this.run("ALTER TABLE ai_user_questions ADD COLUMN answers_json TEXT NOT NULL DEFAULT '[]'");
        }
        this.run("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (135, ?)", new Date().toISOString());
      });
      const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
      if (integrity.some((row) => row.integrity_check !== "ok")) {
        throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
    ...
    const imChainTurnColumns = new Set(
      this.all<{ name: string }>("PRAGMA table_info(im_chain_turns)").map((column) => column.name)
    );
    const imChainTurnSourceMessageIndexPresent = this.get(
      "SELECT 1 AS present FROM sqlite_master WHERE type = 'index' AND name = 'idx_im_chain_turns_source_message'"
    ) !== undefined;
    if (!applied.has(136) || !imChainTurnColumns.has("source_message_id") || !imChainTurnSourceMessageIndexPresent) {
      this.transaction(() => {
        if (!imChainTurnColumns.has("source_message_id")) {
          this.run("ALTER TABLE im_chain_turns ADD COLUMN source_message_id TEXT REFERENCES im_messages(id) ON DELETE SET NULL");
          this.run(
            `UPDATE im_chain_turns
             SET source_message_id = (
               SELECT chain.trigger_message_id FROM im_chains chain WHERE chain.id = im_chain_turns.chain_id
             )
             WHERE source_message_id IS NULL`
          );
        }
        this.run("CREATE INDEX IF NOT EXISTS idx_im_chain_turns_source_message ON im_chain_turns(source_message_id, kind, created_at, id)");
        this.run("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (136, ?)", new Date().toISOString());
      });
      const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
      if (integrity.some((row) => row.integrity_check !== "ok")) {
        throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
    ...
    if (!applied.has(137)) {
      this.transaction(() => {
        const platformColumns = new Set(this.all("PRAGMA table_info(platform_ai_settings)").map((row) => String(row.name)));
        if (!platformColumns.has("system_prompt_override_enabled")) {
          this.run("ALTER TABLE platform_ai_settings ADD COLUMN system_prompt_override_enabled INTEGER NOT NULL DEFAULT 0 CHECK(system_prompt_override_enabled IN (0, 1))");
        }
        const workColumns = new Set(this.all("PRAGMA table_info(work_ai_settings)").map((row) => String(row.name)));
        if (!workColumns.has("system_prompt_override_enabled")) {
          this.run("ALTER TABLE work_ai_settings ADD COLUMN system_prompt_override_enabled INTEGER NOT NULL DEFAULT 0 CHECK(system_prompt_override_enabled IN (0, 1))");
        }
        this.run("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (137, ?)", new Date().toISOString());
      });
      const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
      if (integrity.some((row) => row.integrity_check !== "ok")) {
        throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
    ...
    if (!applied.has(138)) {
      this.transaction(() => {
        // 同一内容哈希可以被多个角色引用；保留原记录和约束，仅移除文件路径唯一限制。
        this.run(`CREATE TABLE character_avatars_shared (
          character_id TEXT PRIMARY KEY REFERENCES characters(id) ON DELETE CASCADE,
          mime_type TEXT NOT NULL CHECK(mime_type IN ('image/jpeg', 'image/png', 'image/webp', 'image/gif')),
          byte_length INTEGER NOT NULL CHECK(byte_length > 0 AND byte_length <= 2097152),
          sha256 TEXT NOT NULL CHECK(length(sha256) = 64),
        ...
        this.run(`INSERT INTO character_avatars_shared (character_id, mime_type, byte_length, sha256, storage_key, width, height, updated_at)
          SELECT character_id, mime_type, byte_length, sha256, storage_key, width, height, updated_at FROM character_avatars`);
        this.run("DROP TABLE character_avatars");
        this.run("ALTER TABLE character_avatars_shared RENAME TO character_avatars");
        this.run("CREATE INDEX idx_character_avatars_storage_key ON character_avatars(storage_key)");
        this.run("INSERT INTO schema_migrations (version, applied_at) VALUES (138, ?)", new Date().toISOString());
        const integrity = this.all<{ integrity_check: string }>("PRAGMA integrity_check");
        if (integrity.some((row) => row.integrity_check !== "ok")) {
          throw new Error(`数据库完整性检查失败：${integrity.map((row) => row.integrity_check).join("；")}`);
        }
