...
describe("数据库版本化迁移", () => {
  it("迁移 125 创建按作品级联清理的加密远程 MCP 配置表", () => {
    ...
    current.run("DELETE FROM schema_migrations WHERE version = 125");
    current.close();
    ...
    expect(migrated.get("SELECT COUNT(*) AS count FROM schema_migrations WHERE version = 125")).toEqual({ count: 1 });
