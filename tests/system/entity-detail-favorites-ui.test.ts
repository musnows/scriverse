...
describe("资料详情收藏界面", () => {
  ...
  it("在可收藏资料的详情标题区把星标放到编辑按钮左侧，并排除种族", async () => {
    ...
    expect(page.text.indexOf('id="setting-editor-pin"')).toBeLessThan(page.text.indexOf('id="setting-editor-edit"'));
    expect(page.text.indexOf('id="character-editor-favorite"')).toBeLessThan(page.text.indexOf('id="character-editor-edit"'));
    expect(page.text.indexOf('id="character-editor-pin"')).toBeLessThan(page.text.indexOf('id="character-editor-edit"'));
    expect(page.text.indexOf('id="knowledge-editor-favorite"')).toBeLessThan(page.text.indexOf('id="knowledge-editor-edit"'));
    expect(page.text.indexOf('id="knowledge-editor-pin"')).toBeLessThan(page.text.indexOf('id="knowledge-editor-edit"'));
    ...
    expect(application.text).toContain('bindEntityDetailPinButton($("#setting-editor-pin"), "setting"');
    expect(application.text).toContain('bindEntityDetailFavoriteButton($("#character-editor-favorite"), "character"');
    expect(application.text).toContain('bindEntityDetailPinButton($("#character-editor-pin"), "character"');
    ...
    expect(application.text).toContain('bindEntityDetailPinButton($("#knowledge-editor-pin"), "organization", () => !isRace && viewOnly ? knowledgeEditorItem : null');
    expect(application.text).toContain('id="draft-dialog-favorite" class="entity-detail-favorite-button"');
    expect(application.text).toContain('id="draft-dialog-pin" class="entity-detail-pin-button hidden"');
    ...
    expect(styles.text).toContain('.entity-detail-pin-button svg { width: 17px; height: 17px; fill: none; stroke: currentColor; stroke-linecap: round; stroke-linejoin: round; stroke-width: 1.6; transform: translateY(.5px); }');
