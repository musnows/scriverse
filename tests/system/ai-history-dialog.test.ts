...
describe("AI 对话历史弹窗", () => {
  ...
  it("将历史列表放入独立弹窗并保留会话切换交互", async () => {
    ...
    expect(page.text).toContain('选择章节和模型后即可开始问答；所有引用都基于已保存正文。');
    expect(page.text).not.toContain('提到续写或润色时会自动加载对应 Skill');
    expect(page.text).not.toContain('/continue-writing 或 /polish-writing 强制加载');
