...
describe("AI 角色扮演界面", () => {
  it("允许为当前对话选择角色卡并展示受限记忆模式", async () => {
    ...
    expect(page).toContain('id="roleplay-memory-dialog" class="dialog wide-dialog roleplay-memory-dialog"');
    expect(page).toContain("该角色的所有角色扮演对话和用户共享同一记忆库");
    ...
    expect(application).toContain("function openRoleplayMemoryDialog(character = characterEditorItem)");
    expect(application).toContain("function openRoleplayMemoryEditor(memory = null)");
    expect(application).toContain("/api/characters/${encodeURIComponent(roleplayMemoryCharacter.id)}/roleplay-memories?");
    ...
    expect(styles).toContain(".character-roleplay-memory-field { display: grid;");
