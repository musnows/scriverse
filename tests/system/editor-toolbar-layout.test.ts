...
describe("编辑器工具栏布局", () => {
  ...
  it("让章节路径独占首行，并统一侧栏工具按钮尺寸", async () => {
    ...
    expect(page.text).toContain('&feature=chapter-save-shortcut-v1');
    ...
    expect(application.text).toContain("remapChapterLineCounts(beforeLineIds, lineIds, chapterAnnotationCounts)");
    expect(application.text).toContain("remapChapterLineCounts(savedLineIds, draftLineIds, savedCounts)");
    ...
    expect(application.text).toContain('import { isGlobalSearchShortcut, isSaveShortcut } from "/keyboard-shortcuts.js?v=20260831-chapter-save-shortcut-v1";');
    expect(application.text).toContain('if (!isSaveShortcut(event) || !state.chapter || state.module !== "editor" || chapterEditorReadOnly || !canEditProse()) return;');
