...
describe("作者完整创作流程", () => {
  ...
  it("首屏书架、大纲伏笔、续写守卫和关系银河图资源完整可达", async () => {
    ...
    expect(application.text).toContain('const stateName = sending ? "queue" : (switching || continuingQuestion) ? "switching" : "send";');
    expect(application.text).toContain('button.disabled = switching || continuingQuestion;');
    expect(application.text).toContain('button.classList.toggle("is-queue", sending);');
