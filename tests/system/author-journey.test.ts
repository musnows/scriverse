...
describe("作者完整创作流程", () => {
  ...
  it("章节概览使用 Toast 且正文编辑区始终占满剩余高度", async () => {
    ...
    expect(styles.text).toContain(".chapter-editor-frame { position: relative; display: flex; flex-direction: column;");
    expect(styles.text).toContain(".chapter-editor-scroll { position: relative; display: grid;");
  ...
  it("首屏书架、大纲伏笔、续写守卫和关系银河图资源完整可达", async () => {
    ...
    expect(page.text).toContain("feature=writing-goal-module-navigation-v2");
    ...
    expect(application.text).toContain('api("/api/platform/ai/usage")');
    expect(application.text).toContain('`/api/works/${state.work.id}/ai-settings/usage`');
    expect(application.text).not.toContain('/usage?timezoneOffset=');
    ...
    expect(application.text).toContain('data-usage-calendar-year aria-label="每日用量年份"');
    expect(application.text).toContain('GitHub 风格网格按服务器时区（${esc(usageTimezone)}）分年展示');
    expect(application.text).toContain('function bindUsageCalendar(root, usage)');
    ...
    expect(aiUsage.text).toContain("export function usageCalendarYears");
    ...
    expect(styles.text).toContain("font: 600 clamp(21px, 2.5vw, 30px)/1 var(--font-latin), ui-monospace, monospace;");
    ...
    expect(application.text).toContain('/ai-usage.js?v=20260830-ai-usage-year-v1');
    ...
    expect(page.text).toContain('id="writing-progress-button" class="module-nav-secondary hidden" type="button"><svg class="nav-icon"');
    expect(page.text).not.toContain('id="writing-progress-button" class="settings-hub-card"');
    expect(page.text.indexOf('id="module-more-button"')).toBeLessThan(page.text.indexOf('id="writing-progress-button"'));
    ...
    expect(application.text).toContain('$("#writing-progress-button").classList.toggle("permission-hidden", Boolean(state.work) && !canReadModule("editor"))');
    ...
    expect(page.text).toContain('feature=writing-goal-module-navigation-v2');
    expect(page.text).toContain('<span class="nav-label">写作目标</span>');
    expect(page.text).toContain('id="module-more-button"');
    expect(page.text).toContain('<span class="nav-label">更多</span>');
    expect(page.text.match(/class="module-nav-secondary hidden/gu)).toHaveLength(8);
    ...
    expect(application.text).toContain("const sendAction = aiSendModeAction(aiComposerSendMode(), sending, aiComposerHasSendablePrompt())");
    expect(application.text).toContain('button.disabled = switching || continuingQuestion;');
    expect(application.text).toContain('button.classList.toggle("is-queue", sendAction === "queue")');
