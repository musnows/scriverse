...
describe("AI 工具调用记录界面", () => {
  it("生成时展开思考与工具步骤并在最终答案出现后折叠", async () => {
    ...
    expect(application).toContain('from "/ai-interactive.js?v=20260915-plan-status-sync-v1"');
    expect(application).toContain("plans.forEach(cacheAiWritePlanDetail);");
    expect(application).toContain("await syncVisibleAiWritePlanCardStatuses();");
    expect(application).toContain("function syncVisibleAiWritePlanCardStatuses()");
    expect(page).toContain("feature=ai-write-plan-status-sync-v1");
