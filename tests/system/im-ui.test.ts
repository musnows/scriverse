...
describe("全局 IM 工作区界面", () => {
  ...
  it("提供全局入口、三栏工作区、模型设置和 typed mention composer", async () => {
    ...
    expect(page.text).toContain("feature=global-im-v67");
    ...
    expect(application.text).toContain('/im.js?v=20260902-global-im-v67');
    ...
    expect(im.text).toContain("if (!committed && current?.id === conversationId)");
    ...
    expect(im.text).toContain("pendingMessageRequest?.content === content");
    expect(im.text).toContain("pendingAnnouncementRequest?.content === content");
