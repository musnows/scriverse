...
describe("全局 IM 工作区界面", () => {
  ...
  it("提供全局入口、三栏工作区、模型设置和 typed mention composer", async () => {
    ...
    expect(page.text).toContain('aria-label="发布公告" title="发布公告"');
    expect(page.text).toContain('class="im-icon-button im-chat-header-icon-button im-button-announcement hidden"');
    ...
    expect(im.text.indexOf("await Promise.all([loadCatalogs(), refreshConversations()]);")).toBeLessThan(im.text.indexOf("!await beforeOpen()"));
    expect(im.text.indexOf("!await beforeOpen()")).toBeLessThan(im.text.indexOf("opened = true;"));
    expect(im.text).toContain('querySelector("#work-meta").textContent = ""');
    expect(styles.text).toContain(".app-shell.im-mode .work-meta { display: none; }");
    expect(im.text).toContain("commitRealtimeMessage(envelope.payload.message)");
    expect(im.text).toContain("data-im-failed-turn");
    expect(im.text).toContain("mergeImFailedReplyPages");
    ...
    expect(im.text).toMatch(/composer\.addEventListener\("keydown", \(event\) => \{\s+if \(event\.isComposing\) return;/);
    ...
    expect(im.text).toContain("presentCharacters().map");
    expect(im.text).toContain("im-member-unavailable");
    ...
    expect(im.text.match(/else conversationDrafts\.delete\(current\.id\);/gu)).toHaveLength(4);
    ...
    expect(page.text).toContain('aria-label="成员与设置" title="成员与设置"');
    expect(page.text).toContain('id="im-details-toggle" class="im-icon-button im-chat-header-icon-button"');
    ...
    expect(im.text).toContain('aria-current="true"');
    expect(im.text).toContain("imConversationAccessibleLabel(item.title");
    ...
    expect(im.text).toMatch(/const detailsFocus = captureGroupSettingsDraft\(\);\s+current = nextConversation;/);
    expect(im.text).toContain("!sameImGroupSettings(latestForm, submittedForm)");
    expect(im.text).toContain("imDiagnosticStatusLabel(turn)");
    expect(im.text).toContain("isImRealtimeChainCurrent(current.activeChain, envelope.payload)");
    expect(im.text).toContain('completed: "未选择 · 低于阈值"');
    ...
    expect(im.text).toContain("const provisionalJudges = new Map()");
    expect(im.text).toContain("syncProvisionalReplies");
    expect(im.text).toContain("syncProvisionalJudges");
    expect(im.text).toContain("upsertProvisionalReply");
    expect(im.text).toContain("upsertProvisionalJudge");
    expect(im.text).toContain("updateProvisionalReplyElement");
    expect(im.text).toContain("updateProvisionalJudgeElement");
    ...
    expect(styles.text).toContain(".im-message.is-provisional.is-judge .im-message-body");
    ...
    expect(styles.text).toContain(".im-details { transition: none !important; }");
