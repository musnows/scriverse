...
describe("全局 IM 工作区界面", () => {
  ...
  it("提供全局入口、三栏工作区、模型设置和 typed mention composer", async () => {
    ...
    expect(page.text).toContain("feature=global-im-v86");
    ...
    expect(application.text).toContain('/im.js?v=20260902-global-im-v86');
    ...
    expect(im.text).toContain('turn.selected ? "已选择" : turn.status');
    expect(im.text).not.toContain('turn.selected ? "已发言" : turn.status');
